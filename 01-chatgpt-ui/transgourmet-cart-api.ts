...
cartStorage.set("jonas.bandi", { positions: [{ articleNumber: "095210", quantity: 1 }, { articleNumber: "022600", quantity: 2 }] });
 ...
 */
function getCartInternal(username: string): Cart {
	return cartStorage.get(username) || { positions: [] };
}
 ...
 */
export async function getCart(username: string): Promise<CartDisplay> {
	console.log("Getting cart for username:", username); 
	const cart = getCartInternal(username);
 ...
 * @returns Updated cart
 */
export function removeFromCart(username: string, articleNumber: string): Cart {
	const cart = getCartInternal(username);
 ...
 * @returns Updated cart
 */
export function updateCartPosition(
	username: string,
	articleNumber: string,
	quantity: number,
): Cart {
	if (quantity <= 0) {
		return removeFromCart(username, articleNumber);
	}
...
export function clearCart(username: string): void {
	cartStorage.set(username, { positions: [] });
}
 ...
 */
export function getCartItemCount(username: string): number {
	const cart = getCartInternal(username);
